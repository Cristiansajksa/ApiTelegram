...
class TelegramApi
{
    ...
    public function downloadFile(string $fileId, string $nameFileForKeep) : void
    {
        ...
        $keepFile = file_get_contents( "https://api.telegram.org/file/bot$this->botToken/{$jsonObject->result->file_path}" );
        file_put_contents( $nameFileForKeep, $keepFile );
    }



    public function editM(int $chatId, string $msg, int $msgId = null, string $buttonsJson = "") : self
    {
    ...
    public function sendD(int $chatId, string $msg, string $nameFile, string $jsonButtons = "") : self
    {
        $postFieldSend = [
